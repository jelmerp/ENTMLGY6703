...
- We will then use said infrastructure to start the exploration of an HTS dataset.
  We'll check out reference genome and HTS read (FASTQ) files,
  and perform FASTQ quality control -- **[the next page](/labs/data.html)**.
...
the analysis can be said to consist of two distinct parts:
...
2. The second can be done on a laptop and only requires some R coding.
...
::: {.callout-note collapse="true"}
#### Side note: What is different on a supercomputer like at OSC? _(Click to expand)_
...
If your account is brand new, you should only have three
^[You can be associated with multiple `PAS` projects and for each one,
a scratch and a project directory is added]:
...
<p align="center"><img src=img_hts/ondemand_files_PAS2250.png width="35%"></p>
...
and compute nodes need to be "reserved",
we need to fill out a form and specify the following details:

- The _OSC Project_ that should be billed for the compute resource usage: `PAS2250`
- The _amount of time_ in hours we want to make a reservation for ^[Note that we'll be kicked off as soon as that amount of time has passed!]: `4`
- The _"working directory"_^[This is the starting location in the file system] for the program:
  your newly-created folder in `/fs/scratch/PAS2250/ENT6703`
- The _version_ of VS Code: `4.8`
...
Here, we can open files like text files including scripts, and images.
...
A computer's **shell** is the interface, inside a Terminal window, that allows you to
...
This is also useful because in many ways,
...
Our prompt at OSC should show the following pieces of information like so:
...
- In a **path** (= location of a file or directory)
  ...
  (With any _leading_ forward slash indicating the computer's _root directory_.)
:::
...
- Anything after a `#` is considered a _comment_ and will be ignored by the shell. 
:::

<hr style="height:1pt; visibility:hidden;" />

### `cd` and command actions and arguments
...
And like many commands that perform and action, it normally has **no output** at all.
...
As pointed out above, paths (dir and file specifications) are **case-sensitive**
on Unix systems!
</details>
...
_(You should still be in `/fs/ess/PAS2250/ENT6703/share`. If not, `cd` there first.)_
...
now the `-h` option is especially useful to see that the files are 21-22 Mb in size:
...
<details><summary>{{< fa user-edit >}} **Your Turn**: Use the command history (up arrows) to repeat the previous exercise.</summary>
</details>

<hr style="height:1pt; visibility:hidden;" />

<details><summary>{{< fa user-edit >}} **Your Turn (Bonus)**: Two periods `..` means the directory "one level up" (towards the computer's root dir), so `cd ..` will move you one dir level up. Try to make use of this move into your personal dir and back to the `share` again. _(Click for the solution)_ </summary>
