...
![](img/rstudio_proj_path.png){fig-align="center" width="35%"}
...
<details><summary>This will produce output in the R console (a lot when loading DESeq2),
and some of it in orange, but all should be good unless you see explicit errors at the bottom (_Click to see expected output_)</summary>
...
metadata_file <- here::here("data/meta/metadata_ed.tsv")
```
...
Like in the _Culex_ paper whose data we are working with,
we will perform a Principal Component Analysis (PCA) and a Differential Expression (DE) analysis
...
The DESeq2 package has its own "object type" (a specific R format type) and
...
_In the two outputs above, note the difference between having the sample IDs as a separate column versus as row names._
:::
...
Here, too, we have to make several modifications: DESeq2 expects an all-numeric matrix
with whole numbers (integers) and with gene IDs as row names:
...
::: exercise
#### {{< fa user-edit >}} Your Turn (_Bonus_)
...
First, we have to normalize the count data to account for differences in library size among samples and "stabilize" the variance among genes^[Specifically, the point is to remove the dependence of the variance in expression level on its mean, among genes]:
...
- _Bonus_: Customize the PCA plot --- e.g. can you "separate" treatment and time
  point (different shapes for one variable, and different colors for the other),
  like in Fig. 1 of the paper?

<details><summary> _Click to see some hints for PCA plot customization_</summary>

To expand on the point of the exercise: in the plot we made above,
...
(as was also done in the paper's Fig. 1).
...
With that `pca_df` dataframe in hand, it will be relatively straightforward to
customize the plot, if you know some ggplot2.
...
Second, we'll extract and store the percentage of variance explained by different principal components,
...
by _controlling for_ time points (and there are different ways to do the latter).
...
- **Play around with the p-value cut-off and the labeling to create a plot you like.**  
  _Check [the vignette](https://bioconductor.org/packages/release/bioc/vignettes/EnhancedVolcano/inst/doc/EnhancedVolcano.html), or the help page (accessed by running `?EnhancedVolcano`) to see how you can do this._
...
- **Figure out the identity of the abovementioned log2-fold change outlier.**\
  _(You can do so either by labeling it in the plot, or by filtering the `res_rc24` table.)_
...
- _Bonus_: Plot the gene with the very high LFC value that we saw when making the volcano plot.
...
::: {.callout-important}
#### Outliers!
You may want to check out the solution to the previous exercise,
...
::: exercise
#### {{< fa user-edit >}} Your Tuen (_Bonus_)
